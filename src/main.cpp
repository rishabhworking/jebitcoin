...
#define REWARD_CUT_BLOCK 15000001
...
uint256 hashGenesisBlock("0x00000ca6b4b0a636002161643e652f56811a48bf327cab0f0c3889949baa82b3");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jebitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jebitcoin Signed Message:\n";
...
{
    // Jebitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 24 * 60 *60; // Jebitcoin: each block
static const int64 nTargetSpacing = 60; // Jebitcoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jebitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Jebitcoin - DarkGravity, written by Evan Duffield - evan@Jebitcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jebitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Jebitcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000052018d8229e91ab01fa6691011ee4a067a7224ad7f98670ca2b53363365");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "Bitcoin Expertise ‘Exploding’ Among Insurance Professionals in 2019: Study";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1553364162;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2197391;

        if (fTestNet)
        {
            block.nTime    = 1553364177;
            block.nNonce   = 275958;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9c1592aeba52a6c0b3921a3bd5928a80f2e59c0a9362c22507621e5adeb3721c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb6, 0xb5, 0xe6, 0xc5 }; 
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JebitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JebitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JebitcoinMiner(CWallet *pwallet)
{
    printf("JebitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Jebitcoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running JebitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JebitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JebitcoinMiner, pwallet));
}
