...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Jebitcoin " + FormatFullVersion();
...
{
    SetThreadPriority(THREAD_PRIORITY_JBTOW_NORMAL);
    while (true)
    {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Jebitcoin is probably already running."), addrBind.ToString().c_str());
        else
